$(document).ready(function () {
    ...
    `;
        ...
        // Bildirimin 5 saniye sonra kaybolmasını sağla
        setTimeout(function () {
    ...
    function AppViewModel() {
        ...
        self.sendAdminNotification = function (message) {
            if (message.trim().length > 0) {
                // Admin'e özel mesaj gönderme isteği
                connection.invoke("SendAdminNotification", message.trim())
                    .then(function () {
                        displayNotification(message); // Mesaj başarılı şekilde gönderildiyse bildirimi göster
                    })
                    .catch(function (err) {
                        console.error("Admin Notification Error: ", err);
                        alert("Sadece adminler bildirim gönderebilir!");
                    });
            }
        };
        ...
        self.sendNewMessage = function () {
            var text = self.message().trim();

            if (text.startsWith("/all ")) {
                var message = text.substring(5); // '/all ' kısmını kaldır
